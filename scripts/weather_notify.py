...
def build_payload(target_date, weekday, route, forecasts):
    title = f"明日の配送天気：{WEEKDAY_LABELS[weekday]}／{'・'.join(route)}"
    lines = []

    for item in forecasts:
        icon, label = weather_label(item["weather_code"])
        lines.append(
            f"{item['name']}：{icon}{label} "
...
def main():
    ...
    target_date, weekday, route = route_for_tomorrow(parse_base_date(args.date))
    forecasts = [fetch_weather(location, target_date) for location in route]
    payload = build_payload(target_date, weekday, route, forecasts)
